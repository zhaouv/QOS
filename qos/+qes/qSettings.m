    ...
    methods
        ...
        function s = loadSSettings(obj,fields)
            ...
            if nargin < 2
                ...
                if ~exist(fullfile(obj.root,obj.user,obj.session,'public'),'file')
                    throw(MException('QOS_qSettings:publicSettingsNotExist',...
						'public settings not found, each session must have a public settings group.'));
                end
...
%             end
        ...
        function saveHwSettings(obj,fields,value)
            ...
            fields_ = {};
            for ii = 1:numel(fields)
                fields_ = [fields_, strsplit(fields{ii},'.')];
            end
            qes.util.saveSettings(obj.root,[{'hardware',selected_hw_settings_group},fields_],value);
        end
        ...
        function DeleteHw(obj)
            %
            devices = qes.qHandle.FindByClass('Hardware');
