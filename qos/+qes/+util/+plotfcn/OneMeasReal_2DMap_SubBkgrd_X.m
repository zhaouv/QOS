function [varargout] = OneMeasReal_2DMap_SubBkgrd_X(Data, SweepVals,ParamNames,MainParam,MeasurementName,AX,IsPreview)
   % plot function for data:
   % OneMeasReal_2DMap_SubBkgrd_X plots 3D spectrum data to a 2D map with color scale
   % for the third dimension, subtract background(mean of the first few data points) along the x axes(the second sweep).
