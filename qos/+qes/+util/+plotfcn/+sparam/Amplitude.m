function [varargout] = Amplitude(Data, SweepVals,ParamNames,MainParam,MeasurementName,AX,IsPreview)
    % plot fonction for S parameter, plot amplitude
    ...
    if length(SweepVals{1}{MainParam(1)}) == 1 % single sweep, single sweep point measurement
        ...
        sz = size(Data);
         if length(sz) == 2 % scalar, array or 2D matrix
            if any(sz == 1) % 1D data, no frequency data
                x = [];
                y = Data(:)';
                z = [];
%                 x_ = real(y);
...
%                     ylabel(AX, ['Im[',MeasurementName{1},']']);
                    ...
                    ylabel(AX, ['Amplitude of ',MeasurementName{1},'(dB)]']);
                end
        ...
        if all(sz > 2) || length(sz) > 2 
            ...
            if any(sz==1)
                if all(sz== 1) % 1D data, each measurement data  point is a scalar
                    ...
                    z = data; 
                    z_ = log10(abs(z))*20;
                    imagesc(x, y, z_','Parent',AX);  % transpose Data to take the convention of spectrum, the first sweep is taken as y
                    set(AX,'YDir','normal');
                    if ~IsPreview
                        colormap(jet);
                        colorbar('peer',AX);
                        xlabel(AX,ParamNames{1}{MainParam(1)});
                        if isnan(freq) % no frequency data
                            ylabel(AX, 'Unknonwn, colorbar: amplitude of data (dB)');
                        else
