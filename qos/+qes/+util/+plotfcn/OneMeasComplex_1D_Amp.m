function [varargout] = OneMeasComplex_1D_Amp(Data, SweepVals,ParamNames,MainParam,MeasurementName,AX,IsPreview)
...
%             elseif length(sz) == 2 % each measurement data  point  is an array
             if length(sz) == 2 % each measurement data  point  is an array
                if any(sz == 1) % 1D data
                    ...
                    error('OneMeasComplex_1D_Amp only handles 1 D data');
                end
