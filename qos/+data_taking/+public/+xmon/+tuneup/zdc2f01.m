...
% arguments order not important as long as the form correct pairs.
    ...
    f01_ini = q.zdc_amp2f_freqUnit*polyval(q.zdc_amp2f01,0);
    function f_ = sweepFreq(bias_)
        f01_est = q.zdc_amp2f_freqUnit*polyval(q.zdc_amp2f01,bias_/q.zdc_amp2f_dcUnit);
    ...
    while true
        ...
        if ~stopBiasForward
            ...
            if numel(f01) <= 3
                zdc_amp2f01_(end) = zdc_amp2f01_(end) - (f01(1) - df)/q.zdc_amp2f_freqUnit;
                r = roots(zdc_amp2f01_);
                % r = sort(r(isreal(r)));
                if isempty(r)
                    throw(MException('QOS_zdc2f01:badInitialZdcAmp2f01Fcn',...
                        sprintf('zdc_amp2f01 setting for qubit %s has no root for %0.3e Hz', q.name,...
                        (f01(1) - df)/q.zdc_amp2f_freqUnit)));
                end
                r = r*q.zdc_amp2f_dcUnit;
                db  = min(abs([r -  bias(1),r -  bias(end)]));
            else
                if polyval(zdc_amp2f01_,bias(end)) >=  polyval(zdc_amp2f01_,bias(end-1))
                    zdc_amp2f01_(end) = zdc_amp2f01_(end) - (f01(end) + df)/q.zdc_amp2f_freqUnit;
                else
                    zdc_amp2f01_(end) = zdc_amp2f01_(end) - (f01(end) - df)/q.zdc_amp2f_freqUnit;
                end
            ...
            e = spectroscopy1_zdc('qubit',qName,'bias',bias(end),'driveFreq',f,'save',false,'gui',false);
            P = [P, e.data];
            ...
            dbForward = db;
            currentF01 = f01(end);
            if abs(last_cal_freq - f01(end)) > RECAL_FREQRANGE
                q.f01 = f01(end);
        ...
        if ~stopBiasBackward
            ...
            if numel(f01) <= 3
                zdc_amp2f01_(end) = zdc_amp2f01_(end) - (f01(1) - df)/q.zdc_amp2f_freqUnit;
                r = roots(zdc_amp2f01_);
                r = sort(r(isreal(r)))*q.zdc_amp2f_dcUnit;
            ...
            bias = [bias(1)+db, bias];
            f = sweepFreq(bias(1));
            e = spectroscopy1_zdc('qubit',qName,'bias',bias(1),'driveFreq',f,'save',false,'gui',false);
            P = [e.data, P];
            ...
            dbBackward = db;
            currentF01 = f01(1);
            if abs(last_cal_freq - currentF01) > RECAL_FREQRANGE
                q.f01 = f01(end);
                piAmp = xyGateAmpTuner('qubit',q,'gateTyp','X','gui',false,'save',false);
                q.g_X_amp = piAmp;
                [iq_center0,iq_center1] =...
        ...
        try
            ...
            if range(f01) > 0.1e6  && num_data_points > 24
                pf = polyfit(bias/q.zdc_amp2f_dcUnit,f01/q.zdc_amp2f_freqUnit,4);
            elseif num_data_points > 12
                pf = polyfit(bias/q.zdc_amp2f_dcUnit,f01/q.zdc_amp2f_freqUnit,2);
            else
                pf = polyfit(bias/q.zdc_amp2f_dcUnit,f01/q.zdc_amp2f_freqUnit,1);
            end
            q.zdc_amp2f01 = pf;
        catch
    ...
    function plotAndSave(bias_,Frequency_,P_,f01_)
        ...
        plot(ax,bi,q.zdc_amp2f_freqUnit*polyval(q.zdc_amp2f01,bi/q.zdc_amp2f_dcUnit),'--','color',[1,1,1],'LineWidth',2);
