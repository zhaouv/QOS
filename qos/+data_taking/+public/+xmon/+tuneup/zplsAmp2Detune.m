...
% arguments order not important as long as the form correct pairs.
	...
	if isempty(q.zpls_amp2f01Df)
		throw(MException('QOS_zdc2f01:invalidInitialValue',...
		'zpls_amp2f01Df empty, zpls_amp2f01Df(1) is taken as the initial value for M(1/M is the modulation cycle of f01 in z pulse bias)'));
	if q.zpls_amp2f01Df(1) == 0
		throw(MException('QOS_zdc2f01:invalidInitialValue',...
		'invalid initial value for M(1/M is the modulation cycle of f01 in z pulse bias), value of M can not be zero.'));
	end
	if q.zdc_amp2fFreqRng < 100e6 || q.zdc_amp2fFreqRng > 2e9
		throw(MException('QOS_zdc2f01:invalidAmp2fFreqRng',...
			'zdc_amp2fFreqRng out of supported range:[100e6, 2e9]'));
	end
    ...
    while true
        ...
        try
            ...
            if range(f01) > 0.2e6  && num_data_points > 20
                pf = polyfit(bias/q.zdc_amp2f_dcUnit,f01/freqUnit,4);
            elseif num_data_points > 10
                pf = polyfit(bias/q.zdc_amp2f_dcUnit,f01/freqUnit,2);
            else
                pf = polyfit(bias/q.zdc_amp2f_dcUnit,f01/freqUnit,1);
