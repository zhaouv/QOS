classdef blochSphere < handle
    ...
    methods
        ...
        function addState(obj,state,traceIdx)
            if ~isa(state,'sqc.qs.state')
                throw(MException('QOS_blochSphere:invalidInput',...
                    'initialState not a quantum state.'));
            end
            if nargin < 3
                traceIdx = 1;
            end
            numTraces = size(obj.states,2);
            if traceIdx > numTraces
                obj.states{end+1} = state;
                traceIdx = numel(obj.states);
            else
                obj.states{traceIdx} = state;
            end
            vs = obj.states{traceIdx}.v;
            a = angle(vs(1));
            obj.theta(traceIdx) = real(2*acos(vs(1)*exp(-1j*a)));
            obj.phi(traceIdx) = real(...
                log((vs(2)*exp(-1j*a))/sin(obj.theta(traceIdx)/2))/1j);
            obj.handles.stateArrows(traceIdx) = NaN;
            obj.plot();
        end
%         function draw(obj)
%             obj.plot();
    ...
    methods (Access = private)
        ...
        function plot(obj)
            if isempty(obj.ax) || ~isvalid(obj.ax)
                hf = qes.ui.qosFigure('Bloch Sphere',false);
                set(hf,'ToolBar','none','MenuBar','none');
                obj.ax = axes('Parent',hf,'Position',[-0.25,-0.25,1.5,1.5],'Visible','off');
                daspect(obj.ax, [1 1 1]);
                if obj.showToolbar
            ...
            zCur = zeros(1,ns);
            for ii = 1:ns
                [xCur(ii) yCur(ii) zCur(ii)] = sph2cart(obj.phi(ii), pi/2-obj.theta(ii), 1);
            end
            for ii = 1:ns
                if ishghandle(obj.handles.stateArrows(ii))
                    obj.handles.stateArrows(ii).delete;
                end
                obj.handles.stateArrows(ii) = qes.ui.mArrow3([0,0,0],[xCur(ii),yCur(ii),zCur(ii)],...
                    'color',[1,0,0],'stemWidth',0.015,...
                    'facealpha',obj.arrorTransparency);
            end
        end
    end
end
